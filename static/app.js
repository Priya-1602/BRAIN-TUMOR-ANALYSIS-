...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // Save analysis result to dashboard automatically
    if (data.patient_id) {
      // Analysis already saved to patients list via backend
      console.log('Analysis saved with patient ID:', data.patient_id);
    }
  } catch (err) {
    console.error(err);
...
downloadBtn.addEventListener('click', async () => {
  ...
  const payload = {
    ...
    patient_id: lastResult && lastResult.patient_id ? lastResult.patient_id : null, // Include patient_id if available
  };
